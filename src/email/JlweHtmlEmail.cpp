...
std::string dinnerOptionsToString(std::string json_str, const std::vector<DinnerUtils::dinner_menu_item> &menu_items) {
    std::string output = "";
    ...
    while (res->next()) {
        ...
        for (unsigned int i = 0; i < dinner_forms.size(); i++) {
            if (dinner_form_id == dinner_forms.at(i).dinner_id) {
                ...
                this->inner_html += "        <p class=\"subline\" style=\"padding-left:30px;\">" + dinnerOptionsToString(res->getString(4), menu_items) + "</p></td>\n";
