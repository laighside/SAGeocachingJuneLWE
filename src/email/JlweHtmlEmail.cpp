...
void JlweHtmlEmail::addCostTable(std::string userKey, sql::Connection *con) {
    ...
    res = prep_stmt->executeQuery();
    if (res->next()){
        int dinner_number_adults = res->getInt(1);
        int dinner_number_children = res->getInt(2);
    ...
    int total_cost = event_cost + camping_cost + dinner_cost + card_surcharge_cost;
...
void JlweHtmlEmail::addCashPayment(std::string payment_total) {
    this->inner_html += "<p>You will need to pay " + Encoder::htmlEntityEncode(payment_total) + " upon arrival at the event. <span style=\"font-weight:bold;\">Note that there will be no change given so please ensure you bring the exact amount.</span></p>\n";
}
...
void JlweHtmlEmail::setHtmlInTemplate(std::string logo_url) {
    ...
    result += "      background: #4FAE27;\n";
    result += "      padding-left: 20px;\n";
    result += "      padding-right: 20px;\n";
    result += "    }\n";
    result += "    #header {\n";
    result += "      text-align: center;\n";
    result += "      background-color: #4FAE27;\n";
    result += "      background-image: linear-gradient(#4FAE27, #FFFFFF);\n";
    result += "      background: #4FAE27;\n";
    result += "      background: linear-gradient(#4FAE27, #FFFFFF);\n";
    result += "    }\n";
    result += "    #header > p {\n";
