...
int main () {
    try {
        ...
        if (jlwe.getPermissionValue("perm_pptbuilder")) { //if logged in
            ...
            std::cout << "<p>The scoring spreadsheet can be downloaded with the button below. It will have the points for each cache and the hide points already set. You then just need to enter the finds from the score books into the spreadsheet. Once complete, it can then be uploaded back to the website which will put the final scores on the powerpoint.</p>\n";
